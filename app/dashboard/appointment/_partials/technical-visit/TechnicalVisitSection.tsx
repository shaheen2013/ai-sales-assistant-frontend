...
import {
  useGetStoreVisitQuery,
  useGetTestDriveQuery,
  useGetTradeInQuery,
  useUpdateStoreVisitStatusMutation,
  useUpdateTestDriveBookingStatusMutation,
  useUpdateTradeInVisitStatusMutation,
} from "@/features/appointmentBooking/appointmentBookingSlice";
...
const TechnicalVisitSection = () => {
  ...
  /*--RTK Query--*/
  const { data: testDriveData, isFetching: testDriveIsFetching } =
    useGetTestDriveQuery(
      {
        limit: 10,
        offset: (page - 1) * 10,
  ...
  const { data: storeVisitData, isFetching: storeVisitIsFetching } =
    useGetStoreVisitQuery(
      {
        limit: 10,
        offset: (page - 1) * 10,
  ...
  const { data: tradeInData, isFetching: tradeInIsFetching } =
    useGetTradeInQuery(
      {
        limit: 10,
        offset: (page - 1) * 10,
  ...
  const [updateTestDriveBookingStatus] =
    useUpdateTestDriveBookingStatusMutation();
  ...
  return (
    <div>
      ...
      <div className="p-4 rounded-2xl outline outline-1 outline-offset-[-1px] outline-gray-50 mt-4">
        ...
        >
          <TabsList className="flex items-center justify-between gap-4">
            <div>
              <TabsTrigger
                value="test_drive"
                className="border-b-2 border-b-transparent data-[state=active]:border-b-2 data-[state=active]:border-b-primary-500 pb-3 transition-all group/technicalVisitTabs"
              >
          ...
          <TabsContent value="store_visit">
            <StoreVisitDataTable
              columns={storeVisitColumns({ handleChangeVisitStatus })}
              data={storeVisitData?.results || []}
              loading={storeVisitIsFetching}
            />
          </TabsContent>
          <TabsContent value="test_drive">
            <TestDriveDataTable
              columns={testDriveVisitsColumns({ handleChangeBookingStatus })}
              data={testDriveData?.results || []}
              loading={testDriveIsFetching}
            />
          </TabsContent>
          <TabsContent value="trade_in">
            <TradeInDataTable
              columns={tradeInColumns({
                handleChangeVisitStatus: handleChangeTradeInVisitStatus,
              })}
              data={tradeInData?.results || []}
              loading={tradeInIsFetching}
            />
          </TabsContent>
        </Tabs>

        {tab === "test_drive" &&
          typeof testDriveData?.count === "number" &&
          testDriveData?.count > 10 && (
            <Pagination
              page={page}
        ...
        {tab === "store_visit" &&
          typeof storeVisitData?.count === "number" &&
          storeVisitData?.count > 10 && (
            <Pagination
              page={page}
              onPageChange={setPage}
        ...
        {tab === "trade_in" &&
          typeof tradeInData?.count === "number" &&
          tradeInData?.count > 10 && (
            <Pagination
              page={page}
              onPageChange={setPage}
