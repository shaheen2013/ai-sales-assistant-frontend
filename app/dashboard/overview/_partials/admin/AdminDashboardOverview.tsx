...
const AdminDashboardOverview = () => {
  ...
  const displayText = !isDifferentDate
    ? "Choose Date Range"
    : `${moment(selection?.[0]?.startDate).format("DD MMM, YYYY")} - ${moment(
        selection?.[0]?.endDate
      ).format("DD MMM, YYYY")}`;
    ...
    <div className="py-2">
      ...
      {adminDashboardFetching ? (
        ...
        <>
          ...
          {/* chart */}
          <div className="">
            {adminDashboardOverviewData?.graph_data && (
              <AdminDashboardOverviewChart
                data={adminDashboardOverviewData?.graph_data}
              />
            )}
          </div>
        </>
