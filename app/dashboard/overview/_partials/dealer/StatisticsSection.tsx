...
export default function StatisticsSection() {
  ...
  useEffect(() => {
    if (dealerCallHistoryStatsData) {
      const formattedData = Object.entries(dealerCallHistoryStatsData?.call_history).map(([key, value]) => ({
        month: key,
        minutes: value,
      }));
        ...
        isDealerCallHistoryStatsLoading ? <DealerCallHistoryStatsSkeleton /> : <div className="border rounded-xl p-4">
          ...
          <div className="grid grid-cols-12 items-center mt-6">
            <div className="col-span-12 xl:col-span-2">
              ...
              <div className="text-gray-700 mt-2"><span className=" text-3xl font-semibold">{dealerCallHistoryStatsData?.current_month?.total_duration} </span><span className="text-lg font-normal">Minute</span></div>
              <div className="text-base font-normal text-gray-500 mt-2"><span className="text-green-500">{dealerCallHistoryStatsData?.current_month?.growth_percentage?.toFixed(2)}%</span> than last Month</div>
              <div className="flex gap-2 justify-start items-start mt-6">
                ...
                <div>
                  ...
                  <div className="text-gray-700 text-base font-medium mt-2">{dealerCallHistoryStatsData?.last_month?.total_duration} Minute</div>
                </div>
              </div>
            </div>
            <div className="col-span-12 xl:col-span-10 h-[347px]">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  />
                  <Bar dataKey="minutes" fill="#55BB78" barSize={32} radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
