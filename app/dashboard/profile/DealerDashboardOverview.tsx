...
import {
  ...
  useGetDealerMinutePlansQuery,
  useUpdateDealerAssistantVoiceMutation,
...
const AdminDashboardOverview = () => {
  ...
  const [selectedMinutePlan, setSelectedMinutePlan] = useState<PlanType | null>(
    null
  );
  ...
  /*--RTK Query--*/
  const {
    data: dealerDashboardOverviewData,
    isLoading: dealerDashboardOverviewLoading,
  } = useGetDealerDashboardOverviewQuery();
  const [updateDealerAssitantVoice] = useUpdateDealerAssistantVoiceMutation();
  const { data: dealerMinutePlans, isLoading: dealerMinutePlansLoading } =
    useGetDealerMinutePlansQuery(undefined, { skip: !modals.addMoreMinute });
  const {
    data: dealerCurrentSubscription,
    isLoading: dealerCurrentSubscriptionLoading,
  } = useGetCurrentSubscriptionPlanQuery(undefined, {
    skip: !modals.addMoreMinute,
  });
  const [
    createMinuteSubscription,
    { isLoading: createMinuteSubscriptionLoading },
  ] = useCreateMinuteSubscriptionMutation();
  ...
  return (
    <div className="py-2">
      {dealerDashboardOverviewLoading ? (
        <DealerDashboardOverviewSkeleton />
      ) : (
        <>
          {/* cards */}
          <div className="grid xl:grid-cols-2 lg:grid-cols-2 grid-cols-1 gap-3 mb-4">
            ...
            <div className="p-4 rounded-xl bg-[#E5DCFA] flex justify-between">
              <div>
                ...
                <h3 className="text-gray-500 text-2xl font-semibold">
                  {dealerDashboardOverviewData?.subscription?.remain_days
                    ? `${dealerDashboardOverviewData?.subscription?.remain_days} Days`
                    : "NA"}
                </h3>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="border-r-2 border-white pr-3">
                  <h2 className="text-gray-400 text-xs text-right">
                    Start Date
                  </h2>
                  <h2 className="text-lg font-semibold">
                    {dealerDashboardOverviewData?.subscription?.start_date
                      ? formateDate(
                          dealerDashboardOverviewData?.subscription?.start_date,
                          "Do MMM, YYYY"
                        )
                ...
                <div>
                  ...
                  <h2 className="text-lg font-semibold">
                    {dealerDashboardOverviewData?.subscription?.end_date
                      ? formateDate(
                          dealerDashboardOverviewData?.subscription?.end_date,
                          "Do MMM, YYYY"
                        )
            ...
            <div className="p-4 rounded-xl bg-[#DDF2F6] flex justify-between">
              <div>
                ...
                <h3 className="text-gray-500 text-2xl font-semibold">
                  {dealerDashboardOverviewData?.talk_time?.remaining
                    ? `${dealerDashboardOverviewData?.talk_time?.remaining} m`
                    : "NA"}
                </h3>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="border-r-2 border-white pr-3">
                  ...
                  <h2 className="text-lg font-semibold">
                    ...
                    {dealerDashboardOverviewData?.talk_time && "m"}
                  </h2>
                </div>
            ...
            <div className="gap-4 grid lg:grid-cols-2 mb-4">
              ...
              <div className="order-2 lg:order-1 border rounded-xl p-4">
                ...
                <h2 className="text-center text-lg font-semibold text-gray-500">
                  {dealerDashboardOverviewData?.phone_number ||
                    "You don't have a Twilio number yet!"}
                </h2>
              ...
              <div className="order-1 lg:order-2 border rounded-xl p-4">
                ...
                <p className="text-sm text-[#202223]">
                  Tap the play button to hear an exciting demo of the voice in
                  action!
                </p>
              </div>
            </div>
      ...
      >
        <DialogContent className="sm:max-w-[1000px] max-h-full overflow-auto">
          ...
          {dealerMinutePlansLoading ? (
            <DealerMinutePlanSkeleton />
          ) : (
            <div className="grid lg:grid-cols-3 gap-4">
                  ...
                  >
                    ...
                    <div className="flex justify-between mb-4">
                      ...
                      <div>
                        <Checkbox
                          id="terms"
                          checked={Boolean(selectedMinutePlan?.id === plan?.id)}
                        />
                      </div>
                    </div>
                    ...
                    {dealerCurrentSubscriptionLoading ? (
              ...
              {minutePlans?.length == 0 && (
                <div className="col-span-3 text-center text-gray-500">
                  No minute plans available. Please contact support for
                  assistance.
                </div>
              )}
            </div>
