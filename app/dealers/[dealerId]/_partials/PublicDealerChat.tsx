...
export default function PublicDealerChat() {
    /*--Next Hooks--*/
    ...
    const selectedDealerInfo: any = useMemo(() => {
        return dealers?.find((dealer: any) => dealer?.user?.dealer_details?.id?.toString() === dealerId);
    }, [dealerId, dealers]);
    ...
    return (
        <div className="h-full overflow-hidden flex flex-col justify-center p-6">
            <div className="flex-1">
                {
                    dealerId && (!initiateChat ? (
                        <AISalesInitializer
                            emailValue={email}
